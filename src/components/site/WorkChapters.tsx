    ...
    <section className="bg-white px-5 py-20 sm:px-8 sm:py-24">
      <div className="mx-auto grid max-w-[1040px] items-start gap-12 md:grid-cols-[1.1fr_0.9fr] md:gap-16">
        <div>
          ...
          <p className="mt-4 max-w-[440px] font-inter text-[16px] leading-[1.6] text-muted sm:text-[17px]">
            {intro}
          </p>
          ...
          <ul className="mt-8 border-t border-[#e2e2dc]">
            {chapters.map((c, i) => {
              const isOpen = open === i;
              return (
                <li key={c.id} className="border-b border-[#e2e2dc]">
                  <button
                    type="button"
                    onClick={() => setOpen(i)}
        ...
        <div className="relative aspect-[4/5] w-full overflow-hidden rounded-[16px] bg-[#f3f2ee] shadow-[0_8px_22px_rgba(0,0,0,0.12)] md:sticky md:top-24">
          <Image
            key={active.image}
            src={active.image}
            alt={active.title}
