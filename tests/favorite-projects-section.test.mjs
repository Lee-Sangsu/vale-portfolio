...
test("FavoriteProjects renders the Figma labels after the Designing for section", () => {
  ...
  const workChaptersIndex = homePage.indexOf("<WorkChapters");
  ...
  assert.ok(workChaptersIndex >= 0, "missing WorkChapters call site");
  assert.ok(designingForSectionEnd < favoriteProjectsIndex);
  assert.ok(favoriteProjectsIndex < workChaptersIndex);
